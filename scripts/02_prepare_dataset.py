...
def parse_args():
    ...
    parser.add_argument(
        "source_dir",
        type=Path,
        help="Raw folder containing labeled data",
    )
    parser.add_argument(
        "output_dir",
        type=Path,
...
def validate_label_line(
    ...
    # Expect 9 values for OBB format: class_id x1 y1 x2 y2 x3 y3 x4 y4
    if len(parts) != 9:
        errors.append(f"{label_file} line {line_number}: expected 9 values, got {len(parts)}")
        return errors
    try:
    ...
    for i in range(1, 9):
        ...
        if not (0.0 <= value <= 1.0):
            ...
            errors.append(f"{label_file} line {line_number}: {coord_name}={value} outside [0,1]")
    return errors
